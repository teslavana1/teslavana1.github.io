    ...
    return (
        <Card elevation={3} sx={{ marginBottom: 5}}>
            <Grid container direction='row' maxHeight={{ md: '450px' }}>
                <Grid container item xs={12} md={6} height='100%' width='100%' maxWidth='100%' zIndex={1}>
                    <Button onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} sx={{ display: 'flex', justifyContent: 'flex-start', height: '100%', width: '100%', p: 0, borderRadius: 0, 
                    // opacity: hover ? .9 : 1.0, 
                    transition: '.3s ease-in-out' }} href={url} onClick={() => setLoading(true)}>
                ...
                <Grid container item direction='column' xs={12} md={6} p={{ xs: 2, md: 3 }} zIndex={10} bgcolor='white'>
                    ...
                    <Grid container item direction='column' wrap={{ xs: 'wrap-reverse', xl: 'wrap' }}>
                        <Grid container item direction='row' xs={12} my={{ xs: 0, md: 2 }} order={{ xs: 3, md: 2 }}>
                            <Typography whiteSpace='break-spaces' variant='body1'>{description}</Typography>
                        </Grid>
